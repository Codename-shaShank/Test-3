...
def write_review_file!(path, content)
  ...
  Dir.mkdir(dir) unless dir.nil? || dir == "." || Dir.exist?(dir)

  File.write(path, content)

  unless File.exist?(path) && !File.empty?(path)
...
if changes.empty?
  ...
  begin
    write_review_file!(review_path, no_change_message)
    puts "No gem changes."
    exit 0
...
begin
  review_path = gem_review_path
  write_review_file!(review_path, markdown)
  puts "Wrote gem review: #{review_path}"
rescue StandardError => e
  warn "Error writing gem_review.md: #{e.message}"
